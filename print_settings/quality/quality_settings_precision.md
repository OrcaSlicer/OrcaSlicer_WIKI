...
Another use case is to reduce the size of the G-code file, which might slow down very very old printers.

Modern printers, printing STL models generated with most modern software, most likely won't benefit from this feature, see details below.
...
> 
> The print quality of properly generated, high resolution STL model might even be reduced, see technical explanation below.
> 
...
> 
> __On most printers the firmware will internally convert the arcs back into several segments__ (at least on Marlin and Klipper printers, covering likely more than 90% of the existing printers), so this feature results in a double conversion: guessing arcs from segments, then then converting arcs back into segments. Since the default settings used by Klipper and by several versions of Marlin are pretty coarse, __a loss of quality will often take place__ except for very low resolution STL models. For example, the segments generated by OnShape with the default preset "Fine" are about 0.6 mm long, while by default Klipper and older Marlin versions use 1.0 mm per segment.
