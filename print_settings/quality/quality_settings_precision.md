...
Another use case is to reduce the size of the G-code file, which would otherwise slow down very old printers limited by SD card read speed.

Modern printers printing STL models obtained with most modern software will not benefit from this feature, see details below.

![arc-fitting](https://github.com/OrcaSlicer/OrcaSlicer_WIKI/blob/main/images/Precision/arc-fitting.svg?raw=true)

> [!IMPORTANT]
> This option is only available for machines that support G2 and G3 commands.
> This features tries to _guess_ arcs from straight lines, and as any guess it won't always be accurate.
> The print quality of properly generated, high resolution STL model might even be reduced, se technical explanation below.
> This feature requires higher CPU usage from the printer microcontroller and therefore it might cause slow-downs (and reduced surface quality) on some printers.

> [!NOTE]
> This feature was initially developed to use CPU power to reduce the amount of data read from a slow SD card, which in old 8 bit printer microcontrollers was often the bottleneck. STLs were also in the past generated at very low resolution.
> _On most printers the firmware will internally convert the arcs back into several segments_ (at least on Marlin and Klipper printers, covering likely more than 90% of the existing printers), so this feature results in a double conversion: guessing arcs from segments, then then converting arcs back into segments. Since the default settings used by Klipper and by several versions of Marlin are pretty coarse, _a loss of quality will take place_ except for very low resolution STL models. For example, the segments generated by OnShape with the default preset "Fine" are about 0.6 mm long, while by default Klipper and older Marlin versions used 1.0 mm per segment.
