...
.DESCRIPTION
    Convenience wrapper that chains the three repository maintenance steps:
...
.PARAMETER DryRun
    Preview mode: image fixer and option sync run with -DryRun, and mkdocs.yml is previewed (not written).
...
foreach ($required in @($fixScript, $syncScript, $navScript)) {
...
if (-not $python) {
    throw "python (or python3) is required for step 3 but was not found on PATH."
}
